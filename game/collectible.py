...
class Collectible(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Random movement with speed adjusted by size
        self.direction = random.uniform(0, 2 * math.pi)
        # Larger collectibles move slower
        self.speed_multiplier = COLLECTIBLE_BASE_SIZE / self.size
        self.speed = COLLECTIBLE_SPEED * self.speed_multiplier
        self.velocity_x = math.cos(self.direction) * self.speed
        self.velocity_y = math.sin(self.direction) * self.speed
